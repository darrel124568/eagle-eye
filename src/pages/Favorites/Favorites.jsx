    ...
    <main className="mx-auto max-w-7xl space-y-8 px-5 py-10 sm:px-8 sm:py-12">
      ...
      {favorites.lenght === 0 ? (
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {favorites.map((bird) => (
        <li key={bird.scientific_name} className="flex flex-col overflow-hidden rounded-2xl border border-blue-100 bg-white p-5 shadow-sm shadow-blue-950/5">
          <Link
          ...
          <button onClick={() => RemoveFavorite(bird.scientific_name, favorites, setFavorites)} className="mt-3 inline-block w-full rounded-xl border border-blue-200 py-2.5 text-center text-sm font-semibold text-blue-800 transition-colors hover:bg-blue-50">
